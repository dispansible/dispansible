...
if [ "$OS_FAMILY" == "Darwin" ]; then
  ...
  if [ "$POSTIVIE_CHECK" != "$EXPECTED_VBOX_VERSION" ]; then
    echo "virtualbox is not installed as expected. Version found: $POSITIVE_CHECK instead of $EXPECTED_VBOX_VERSION!"
    exit 1
  fi
fi
